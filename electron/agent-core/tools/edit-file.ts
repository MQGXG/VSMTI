...
export const editFileTool = make({
  name: "edit_file",
  description: "Replace exact text in one file. Uses fuzzy matching with 9 fallback strategies: exact → trimmed lines → block anchors → whitespace → indentation → escape normalization → trimmed boundary → context-aware → multi-occurrence.",
