  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-medium" style={{ color: '#E8F4F0' }}>模型</h3>
          <p className="text-xs mt-1" style={{ color: '#5C8D8A' }}>管理所有可用模型，启用后可在对话框中选择使用</p>
        </div>
        <div className="text-xs" style={{ color: '#5C8D8A' }}>已启用 {enabledCount} / {totalCount}</div>
      </div>

      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: '#5C8D8A' }} />
        <input
        ...
        {Object.entries(grouped).map(([providerName, models]) => (
          <div key={providerName} className="space-y-2">
            <div className="flex items-center gap-2 px-1">
              <Cpu className="w-4 h-4" style={{ color: '#5C8D8A' }} />
              <span className="text-sm font-medium" style={{ color: '#E8F4F0' }}>{providerName}</span>
              <span className="text-xs" style={{ color: '#3A5A58' }}>({models.length})</span>
            </div>
              ...
              {models.map((model) => (
                <div key={`${model.providerId}-${model.id}`}
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full" style={{ background: model.enabled ? '#00D9C0' : '#1A2E35' }} />
                    <div>
                      <div className="text-sm" style={{ color: '#E8F4F0' }}>{model.name}</div>
                      <div className="text-xs font-mono" style={{ color: '#3A5A58' }}>{model.id}</div>
                    </div>
                  </div>
                  <div onClick={() => toggleModel(model.providerId, model.id)}
                    className="w-11 h-6 rounded-full relative cursor-pointer transition-colors"
                    style={{ background: model.enabled ? '#00D9C0' : '#1A2E35' }}>
