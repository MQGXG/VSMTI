    ...
    async (content: string) => {
      ...
      try {
        ...
        const tools = await window.electronAPI.agent
          ...
          .catch(() => []);
        if (tools.length > 0) {
          const { routeToolMessage } = await import("@/components/chat/tool-router");
