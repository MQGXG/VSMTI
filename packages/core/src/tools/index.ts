/**
 * 所有工具统一导出 — 类似 OpenCode 的 application-tools.ts
 * 新增工具只需在这里加一行
 */

export { writeFileTool } from "./write-file"
export { listFilesTool } from "./list-files"
export { webSearchTool } from "./web-search"
export { webFetchTool } from "./web-fetch"
export { codeSearchTool } from "./code-search"
export { questionTool } from "./question"
export { grepTool } from "./grep"
export { globTool } from "./glob"
export { codeExecTool } from "./code-exec"
export { bashTool } from "./bash"
export { editFileTool } from "./edit-file"
export { gitStatusTool, gitDiffTool, gitLogTool, gitCommitTool } from "./git"
export { createDocxTool } from "./create-docx"
export { searchHistoryTool } from "./search-history"
export { memorySearchTool, memoryRecallTool } from "./memory"
export { spawnAgentTool, waitAgentsTool, listSubagentsTool } from "./agent-tools"
export { createMCPTool } from "../mcp/index"
